        ...
        >
            ...
            <div class="role" v-for="(value,index) in Datagroup" :key="index" @click="choose(index)">
                <label :for="index">{{ value.F_RoleName }}
                    <input type="radio" :name="value" :id="index"  :checked ="checkeda==index">
                </label>
            </div>
        </van-popup>
        ...
        >
            ...
            <div class="siteSelection" v-for="(value,index) in Datesite" :key="index">
                <div class="siteSelection_div">
                    <div class="siteSelection1">{{ value.name }}</div>
                    <van-switch
...
export default {
    ...
    methods: {
        ...
        vanfield(){
            ...
            axios({
                ...
                for(let i in this.Datagroup){
                    this.Datagroup[i].F_RoleId
                    if(this.Datagroup[i].F_RoleId == this.F_RoleId){
                        this.checkeda = i
                    }
                }
            }).catch(err=>{
        ...
        siteSelection(index){
            ...
            //     item.status == true ? this.checked = true : this.checked = false
                
            // });
            
            var check ={
                quanbu:this.Datesite[index].status
            }
            this.data.push(check)
        },
        //确定场地选择
        ...
        save(){
            ...
            if(!/^((1[3,5,8,9,7][0-9])|(14[5,7])|(17[0,6,7,8])|(19[7]))\d{8}$/.test(this.username)){
                ...
                for(let i in this.Datesite){
                    if(this.Datesite[i].status == true){
        ...
        bianji(){
            ...
            axios({
                mounted:'GET',
                url:url.adminurl+'/api/AccountApi/Detail?accountId='+accountId
            }).then((err)=>{
                this.username = err.data.accountInfo.mobile
                this.name = err.data.accountInfo.name
                this.Datesite = err.data.addressInfo
                this.role = err.data.roleInfo.F_RoleName
                this.password = err.data.accountInfo.pwd
                for(let i in this.Datesite ){
                    if(this.Datesite[i].status == 1){
                        this.Datesite[i].status = true
                        this.data.push(this.Datesite[i].id)
                        if(this.data.length == 0){
                            return false
                        }else{
                            this.permissions ="已选择"
                ...
                this.F_RoleId = err.data.roleInfo.F_RoleId
