		...
		<div class="revise-list" v-for='(item,index) in listData' :key='index'>
			<div class="list-div">
				...
				<div class="li-right" tag='div' >
					...
					<router-link @click='goodInfo'  :to="{path:'/goodrevise',name:'goodrevise',params:{lid:item.id}}">
						<van-button round type="default" size="small" class='li-btn' >商品修改</van-button>
					</router-link>
				</div>
			</div>
		</div>
...
//   	},
  ...
  //挂载完成
  mounted(){
	//   this.http.get(url.adminurl+'/api/ProductTypeApi/ListAll').then(res=>{
    //     console.log(res)
	// }) 
