					...
					<div class="li-goods-div" v-for="(item,index) in goodslist" :key="index">
						...
						<div class="good-info">
							...
							<div class="good-stock">
								<span>现有库存：{{item.remainder}}</span>
								<span>货道容量： {{item.maxSize}}</span>
							</div>
						</div>
					</div>
					<van-pagination 
			...
			<van-tabs v-model="active" animated @click="onClick">
				...
				<!-- <van-tab title="其他商品">
					...
					<div class="goods-lists">
						<van-pull-refresh v-model="isLoading" @refresh="onRefresh">
							<van-list v-model="loading" :finished="finished" @load="onLoad">
								<div class="li-goods-div" v-for='(item,index) in otherListData' :key='index'>
									<div class="li-goods" @click="handleGood(item.id)" :data-goodid = "index">
										<div class="good-img">
											<img :src="item.imgUrl" alt="" />
										</div>
...
export default {
	...
	methods: {
		...
		// 	axios({
		// 		method: 'get',
		// 		url:url.adminurl+'/api/MachineBoxApi/List?machineId='+_this.$route.params.manageid,
		// 	}).then((res)=>{
		...
		// 	axios({
		// 		method: 'get',
		// 		url:url.adminurl+'/api/ProductApi/List?page='+this.pno+'&status=0',
		// 	}).then((res)=>{
		...
		/**------------ 点击商品(end) --------------*/
		 ...
		 init() {
			if(this.tabIndex==0){
				...
				axios({
					method: 'get',
					url:url.adminurl+'/api/ProductApi/List?page='+pageNumber+'&status=0',
				}).then((res)=>{
				...
				axios({
					method: 'get',
					url:url.adminurl+'/api/ProductApi/ListCommon?page='+pageNumber,
				}).then((res)=>{
					_this.isLoading = false; //关闭下拉刷新效果
					_this.otherListData=res.data.data;
